...
to setup
  clear-all
  ask patches [set totalInfectionsHere 0]
   set infected 0
   set immune 0
   set susceptible 0
   set population 0
...
to infect ;; turtle procedure
  ask other turtles-here with [ not sick? and not immune? ]
    [ if random-float 100 < infectiousness
      [ get-sick
        ask patch-here [set totalInfectionsHere totalInfectionsHere + 1]
  ] ]
end
...
to counter
 ...
 set susceptible count turtles with [sick?]
 set immune count turtles with [immune?]
 set infected count turtles with [not sick? and not immune?]
