...
The [`{d6berlin}` package](https://github.com/EcoDynIZW/d6berlin) provides several functions for the members of the Ecological Dynamics Department of the IZW. Now two functions are added to the package: `download_fisbroker_wfs()` and `download_fisbroker_atom()`.
...
![](man/fisbroker_site_wfs_comb_new.PNG)

```{r example wfs, fig.width=8}
...
ggplot() +
  ...
  rcartocolor::scale_fill_carto_d(palette = "Bold", name = "District") +
  theme_void()
```
