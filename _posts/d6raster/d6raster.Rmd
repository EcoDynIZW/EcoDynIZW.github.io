---
title: "a function to load data from the fisbroker library"
description: |
 In the d6raster package is a function called download_fisbroker that downloads wfs data from the fisbroker library from the Berlin Senat. 
...
The [`{d6raster}` package](https://github.com/EcoDynIZW/d6raster) provides a functzion called download_fisbroker. aims provide template maps for Berlin with carefully chosen and aesthetically pleasing defaults. Template maps include green spaces, imperviousness levels, water bodies, district borders, roads, and railways, plus the utility to add a globe with locator pin, a scalebar, and a caption to include the data sources. 
