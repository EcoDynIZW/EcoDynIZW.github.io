---
title: "How to Import Rectangular, Plain-Text Data Files"
description: |
  Learn how to import (and export) various data file formats using a range of packages such as {utils}, {vroom}, {readr}, {data.table}, {readxl}, and {rio}. The article also includes best practices when specifying paths and a quick discussion on handling R specific formats such as RDS and Rdata.
categories:
  - rstats
  - workflow
  - tutorial
...
There is one essential step to make use of the power of the R programming language to wrangle, analyze, visualize, and communicate our data: getting the data into R. In this blog post, we will show you multiple approaches for various rectangular, plain-text file formats. Note that this blog post is focusing on rectangular data --- if you need to import spatial data files, have a look [here](ADD URL).
...
-  something like `C:\\Username\\Documents\\...\\data.csv` or `/Users/Username/Documents/.../data.csv`
-  for Windows paths you have to change the backslashes to forward slashes or escape them by using two backslashes
...
`read.table()` is a generic function that can be used for all kinds of rectangular data formats; `read.csv()` is a short-hand wrapper that calls `read.delim()` with sensible defaults for CSV files.
...
```{r}
dat <- data.table::fread(path_to_data, colClasses = c(code = "factor", year = "factor")) ## as in read.*()
dat <- data.table::fread(path_to_data, colClasses = list(factor = c("code", "year")))    ## by type
dat <- data.table::fread(path_to_data, colClasses = list(factor = 2:3))                  ## by type with column numbers
```
...
```{r, eval=FALSE}
dat <- readxl::read_xlsx("data/data.xlsx", sheet = 2) ## open second sheet
dat <- readxl::read_xlsx("data/data.xlsx", sheet = "Table 2") ## open sheet named "Table 2"
```
...
```
When passing a CSV file as in this example, the `import()` function uses `fread()` from the `{data.table}` package by default. The `import()` functions supports all kind of rectangular data and also other file format including SAS, SPSS, Stata, R Objects, json, geojson, Apache Arrow, Feather and more.


### Working with R-specific Formats

R provides two file formats for storing data: `.RDS` and `.RData`. RDS files, formerly known as RDA files which one should avoid nowadays, can store a only single R object while RData files can store multiple R objects. Another important difference is that RDS files need to be assigned while Rdata files are loaded and use the object name that has been picked by the person saving the file (for good or worse, as it may overwrite existing objects in your environment).
