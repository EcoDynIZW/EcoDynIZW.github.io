...
# in terminal run ollama pull qwen2.5-coder:7b

# then ollama run qwen2.5-coder:7b

# if not working check graphic driver. needs the newest

# add shortcut ctrl + shift + G for running prompt


```{r}
library(llmcoder)
llmcoder_setup("ollama", model = "codestral")
# Load the mtcars dataset and create a scatter plot of mpg vs wt, colored by number of cylinders# Load the mtcars dataset

library(tidyverse)

mtcars %>%
  ggplot(x = wt, y = mpg, color = factor(cyl)) +
  geom_point() +
