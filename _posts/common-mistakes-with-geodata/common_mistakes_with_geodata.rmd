...
```{r, echo=FALSE}
#main_dir <- "C:/Users/wenzler/PopDynIZW Dropbox/Teaching/_TU_BiodivDynamics"
main_dir <- "C:/Users/wenzler/PopDynCloud/Teaching/_TU_BiodivDynamics"
```
...
subdir_berlin <- paste(main_dir, "_DataBerlin", sep = "/")
subdir_tif <- paste(subdir_berlin, "Raster_geotiff", sep = "/")
subdir_asc <- paste(subdir_berlin, "Raster_ascii", sep = "/")
subdir_animals <- paste(subdir_berlin, "Tiererfassungsdaten", sep = "/")
subdir_shape <- paste(subdir_berlin, "Shapefiles", sep = "/")
```

# Load data

Now, we load the necessary datasets. When naming variables, it is best practice to avoid capital letters and use short, descriptive names for ease of typing and readability.

```{r data}
# excel/csv files
boar <- read.csv(paste(subdir_animals, 
                       "PortalBeee_data_wb_melden_utf8_clean.csv", 
                       sep = "/"))
head(boar)
```
...
## Set the projection for an ASC file
...
```{r}
bezirke_sf <- st_read(paste(subdir_shape ,"polygon_bezirke_utf8_32633.shp", sep = "/"))
bezirke_sf
```
...
```{r}
bezirke_sf_4326 <- st_transform(bezirke_sf, 4326)

#synonyms:
#bezirke_sf_4326 <- st_transform(bezirke_sf, st_crs(boar_sf)$proj4string)
#bezirke_sf_4326 <- st_transform(bezirke_sf, st_crs(boar_sf)$epsg)
```
