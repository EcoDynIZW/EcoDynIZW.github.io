...
description: |
  Learn how to efficiently use the powerful graphics library ggplot2 by avoiding repeated code and manual adjustments. You will leanr how to define your themes globally and how to use the {patchwork} package to create multi-panel plots with automated numbering and combined legends.
categories:
  - rstats
  - ggplot2
  - workflow
...
By default, both plots take the same space. In case you want to adjust how the plots are laid out, use `plot_layout()` in combination with either `widths` or `heights`. These arguments take a vector with the relaitve width or height for each plot, repsectively.
