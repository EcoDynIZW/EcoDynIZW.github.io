...
You can use any vector data with the same projection and extent to extract values from a raster. The `extract()` function 

```{r}
pts <- tibble(x = c(4528790, 4576890),
              y = c(3271740, 3272140))

dummy_line <- st_as_sf(pts,
                       coords = c("x", "y"),
                       crs = 3035,
                       sf_column_name = "geometry")


terra::extract(x = temp_rast, 
               y = dummy_line, 
               fun=NULL, 
               method="simple", 
               cells=FALSE, 
